...
public class StatusBar extends GameObject {
	...
	private void paintHP(Graphics g, int x, int y) {
		...
		g.drawRoundRect(x + MARGIN_FONT_BAR + ((hpWidth > mpWidth) ? hpWidth : mpWidth), y, widthPerBar, heightPerBar,
				30, 30);

		String s = status.getCurrentHP() + " / " + status.getTotalHP();
...
//        g.drawString(s, 110, 40); 
	...
	private void paintMP(Graphics g, int x, int y) {
		...
		String s = status.getCurrentMP() + " / " + status.getTotalMP();
		g.setFont(new Font("Helvetica", Font.BOLD, 12));
//        g.drawString(s, 110, 75); 
		g.drawString(s, x, y);
	}
