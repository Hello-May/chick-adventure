...
public class DirectionGroup extends GameObject {
	...
	public DirectionGroup(int x, int y, int width, int height, OnCompleteListener onCompleteListener) {
		...
		animaDelayCounter = new DelayCounter(300 / Global.MILLISEC_PER_UPDATE, true);// 橘燈
		timeLimitCounter = new DelayCounter(2000 / Global.MILLISEC_PER_UPDATE);
		// 先跑timeLimitCounter才跑animaDelayCounter相加總合=dg出現的時間
	...
	public void move() {
		if (animaDelayCounter.updatePause()) {// 更新後
			onCompleteListener.failed();// 才會把dg刪掉
		}
		if (!comboNotes.isReady() | timeLimitCounter.update()) {// 沒ready按錯的時候
			animaDelayCounter.start();// 開始跑
